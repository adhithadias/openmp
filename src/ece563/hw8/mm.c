...
void printArray(double* a, int rows, int cols, int sparseness) {
   ...
   fflush(stdout);
   for (int i=0; i<rows && i<4; i+=sparseness) {
      for (int j=0; j<cols && j<4; j+=sparseness) {
...
double* makeArray(int rows, int cols, int pid) {
...
MPI_Datatype makeType( ) {
   // fill this in for the homework
   MPI_Datatype blockType;
   MPI_Type_vector(LOCAL_ARRAY_ROWS, LOCAL_ARRAY_COLS, LOCAL_ARRAY_COLS,
                  MPI_DOUBLE, &blockType);
   MPI_Type_commit(&blockType);
   return blockType;
}

int calculateShift(int current, int shiftAmount, int max) { // shift is always 
                                                            // up or left
...
int main (int argc, char *argv[]) {
   ...
   double* a = makeArray(LOCAL_ARRAY_ROWS, LOCAL_ARRAY_COLS, pid);
   double* b = makeArray(LOCAL_ARRAY_ROWS, LOCAL_ARRAY_COLS, pid);
   double* sendBuffer = makeArray(LOCAL_ARRAY_ROWS, LOCAL_ARRAY_COLS, pid);
   double* c = makeArray(LOCAL_ARRAY_ROWS, LOCAL_ARRAY_COLS, pid);
   ...
   // if(pid == 0) {
   //    printf("c matrix final answer\n");
   //    printArray(c, LOCAL_ARRAY_ROWS, LOCAL_ARRAY_COLS, 1);
   //    fflush(stdout);
   // }
